...
if (!fs.existsSync("profile.txt")) {
  ...
  fs.writeFileSync("profile.txt", texttowrite, function () {
    return;
  });
}
...
async function waiter() {
  ...
  for (i = 0; i < 10000; i++) {
    if (ready) {
      sdk.swarm.join(
        Buffer.from(
          "3b9f8ccd062ca9fc0b7dd407b4cd287ca6e2d8b32f046d7958fa7bea4d78fd75",
          "hex"
        )
      );
      break;
...
const requestListener = function (request, response) {
  ...
  if (request.method === "GET") {
    ...
    if (parts.path.startsWith("/status")) {
      ...
      sendResponse(response, status, 200, { "Content-Type": "text/plain" });
    }
    ...
    if (parts.path == "/profile") {
      collectData(request, async (formattedData) => {
        ...
        if (old_profile_text == formattedData) {
          sendResponse(response, "No change", 200, {
            "Content-Type": "text/plain",
          });
          return;
        }
        ...
        sendResponse(response, profile_is_set, 200, {
          "Content-Type": "text/plain",
        });
      });
    }
...
if (
  ...
  await publicDrive.put(
    "/profile.json",
    b4a.from(JSON.stringify(profile_to_set))
  );
...
const server = new Server(slashtag, {
  ...
  onauthz: async (token, remote) => {
    if (!isValidUser(remote))
      return { status: "error", message: "sign up first!" };
